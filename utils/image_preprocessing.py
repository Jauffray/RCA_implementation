        ...
        with warnings.catch_warnings():
            ...
            with np.errstate(divide='ignore'):
                ...
                if cropped.shape[1]>512:
                    return Image.fromarray(cropped), Image.fromarray(255*fov_im.astype('uint8'))
                else:
                    return pil_image, Image.fromarray(255*np.ones(pil_image.size).astype('uint8'))
    except:
        return pil_image, Image.fromarray(255*np.ones(pil_image.size).astype('uint8'))
...
def estimate_tr(imgar, A, w=0.95):
    """
...
def correct_illumination(RGBImage, radius=20, r_guid=20, eps_guid=0.1):
    ...
    return skimage.img_as_ubyte(restored)
...
def color_transfer_from_stats(source_stats, target):
#     source = np.array(source)[:, :, ::-1]
    target = np.array(target)[:, :, ::-1]
    target = cv2.cvtColor(target, cv2.COLOR_BGR2LAB).astype("float32")
